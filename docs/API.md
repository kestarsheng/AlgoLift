...
前端待办入口为 `/todos`，需要 JWT 登录。页面支持关键词、状态、优先级和逾期筛选；新增与编辑共用表单，完成操作使用 PATCH，删除需二次确认。列表加载、空列表、保存和删除失败均有对应提示，逾期项标红，已完成项降低视觉强调。
...
Dashboard 数据概览入口为 `/dashboard`（根路径 `/` 默认重定向），由前端并行组合现有分类、题目、错题、笔记、待办和进度接口的分页总数与列表数据；待办总数、未完成数和逾期数分别使用对应筛选查询。各模块独立记录错误，单个接口失败时页面仍展示其余模块并提示部分失败；全部数据为空时展示引导空状态。
