...
export const createNote = async (userId: string, input: NoteInput) => ({ data: convert(await prisma.note.create({ data: { userId, title: input.title.trim(), content: cleanMarkdown(input.content), solutionLinks: input.solutionLinks as unknown as Prisma.InputJsonValue }, select })) });
export const updateNote = async (userId: string, id: string, input: NotePatch) => { try { return { data: convert(await prisma.note.update({ where: { id, userId }, data: { ...(input.title === undefined ? {} : { title: input.title.trim() }), ...(input.content === undefined ? {} : { content: cleanMarkdown(input.content) }), ...(input.solutionLinks === undefined ? {} : { solutionLinks: input.solutionLinks as unknown as Prisma.InputJsonValue }) }, select })) }; } catch (error: unknown) { return missing(error); } };
