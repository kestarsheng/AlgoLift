<!-- 练习记录列表：展示日期、序号、是否一遍做对与备注，支持新增、编辑和删除。 -->
...
const editForm = reactive({ practicedAt: '', solvedFirstTry: true, remark: '' });
...
const startEdit = (record: { id: string; practicedAt: string; solvedFirstTry: boolean; remark: string | null }): void => {
  editingId.value = record.id;
  editForm.practicedAt = record.practicedAt.slice(0, 10);
  editForm.solvedFirstTry = record.solvedFirstTry;
  editForm.remark = record.remark ?? '';
  store.error = '';
};
const saveEdit = async (recordId: string): Promise<void> => {
  if (!editForm.practicedAt) return;
  try {
    await store.update(recordId, { practicedAt: editForm.practicedAt, solvedFirstTry: editForm.solvedFirstTry, remark: editForm.remark.trim() || null });
...
<template>
  <div class="rounded-[3px] border border-[var(--color-border)] bg-[var(--color-surface)] p-3.5">
    ...
    <div v-else-if="store.error && !editingId" role="alert" class="rounded-[3px] border border-[color-mix(in_srgb,var(--danger)_35%,var(--color-border))] p-3 text-sm text-[var(--danger)]">{{ store.error }}</div>
    <p v-else-if="!store.items.length" class="py-6 text-center text-[15px] text-[var(--color-text-muted)]">还没有练习记录，点击右上角「记一次练习」开始记录。</p>
    <div v-else>
      <div v-for="(record, i) in store.items" :key="record.id">
        <div v-if="editingId === record.id" class="flex flex-wrap items-end gap-3 border-t border-[var(--color-border)] py-[9px] first:border-t-0">
          <label class="text-sm text-[var(--color-text-secondary)]">
            练习日期
            <input v-model="editForm.practicedAt" type="date" required aria-label="练习日期" class="mt-1 block rounded-[3px] border border-[var(--color-border)] bg-[var(--color-surface)] px-2.5 py-1.5 text-[var(--color-text)] outline-none focus:border-[var(--color-accent)]" />
          </label>
          <label class="flex items-center gap-1.5 pb-1.5 text-sm text-[var(--color-text-secondary)]">
            <input v-model="editForm.solvedFirstTry" type="checkbox" class="accent-[var(--color-accent)]" />
            一遍做对
          </label>
          <label class="min-w-[180px] flex-1 text-sm text-[var(--color-text-secondary)]">
            备注
            <input v-model="editForm.remark" aria-label="练习备注" placeholder="备注（可选）" class="mt-1 block w-full rounded-[3px] border border-[var(--color-border)] bg-[var(--color-surface)] px-2.5 py-1.5 text-[var(--color-text)] outline-none focus:border-[var(--color-accent)]" />
          </label>
          <button :disabled="store.saving" type="button" class="rounded-[3px] bg-[var(--color-accent)] px-[11px] py-[5px] text-sm font-medium text-white transition-colors hover:bg-[var(--color-accent-hover)] disabled:opacity-50" @click="saveEdit(record.id)">{{ store.saving ? '保存中…' : '保存' }}</button>
          <button type="button" class="rounded-[3px] border border-[var(--color-border)] px-[11px] py-[5px] text-sm text-[var(--color-text-secondary)] transition-colors hover:bg-[var(--color-hover)]" @click="cancelEdit">取消</button>
        </div>
        <div v-else class="grid grid-cols-[auto_auto_1fr] items-center gap-2.5 border-t border-[var(--color-border)] py-[9px] first:border-t-0 sm:grid-cols-[auto_auto_auto_minmax(0,1fr)] sm:gap-2.5">
