<!-- 活动热力图：119 天按周日列首对齐，5 级色阶，今天高亮，hover 显示日期与次数。 -->
...
<template>
  <div v-if="days.length" class="overflow-hidden">
    <div class="mb-0.5 flex h-4 gap-[3px] pl-[30px]">
      <span v-for="(label, index) in monthLabels" :key="index" class="text-center font-mono text-xs leading-4 text-[var(--color-text-muted)]" :style="{ flex: label.span }">{{ label.month >= 0 ? label.month + 1 : '' }}</span>
    </div>
    <div class="flex gap-[3px]">
      ...
      <div class="flex flex-1 justify-start gap-[3px]">
        <div v-for="(week, weekIndex) in weeks" :key="weekIndex" class="flex flex-1 flex-col gap-[3px]">
          <div v-for="(day, dayIndex) in week" :key="dayIndex" class="min-h-[14px] flex-1 rounded-[2px] transition-transform" :class="[heatClass(day.level), day.level < 0 ? 'cursor-default' : 'cursor-pointer hover:scale-[1.35] hover:z-10', day.date === todayDate ? 'outline-[1.5px] outline outline-offset-[-1px] outline-[var(--color-accent)]' : '']" :title="cellTitle(day)" />
        </div>
      </div>
    </div>
