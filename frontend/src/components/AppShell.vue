...
const navigation = [{ label: '刷题练习', path: '/categories' }, { label: '错题本', path: '/wrongs' }, { label: '题解笔记', path: '/notes' }, { label: '待办', path: '/todos' }];
const activePath = computed(() => route.path.startsWith('/categories') || route.path.startsWith('/problems') ? '/categories' : route.path.startsWith('/notes') ? '/notes' : route.path.startsWith('/todos') ? '/todos' : '/wrongs');
</script>
<template>
  <div v-if="auth.isAuthenticated" class="min-h-screen md:grid md:grid-cols-[220px_1fr]">
    <aside class="border-b border-[var(--color-border)] bg-[var(--color-bg)] p-4 md:min-h-screen md:border-b-0 md:border-r md:p-6">
      ...
      <div class="mt-4 flex items-center justify-between gap-3 text-sm"><span>{{ auth.user?.displayName || auth.user?.email }}</span><button class="underline" @click="logout">退出登录</button></div><nav class="mt-5 flex gap-2 md:mt-8 md:block md:space-y-2" aria-label="主导航">
